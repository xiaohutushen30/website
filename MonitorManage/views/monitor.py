...
def HistoryData(request, SN):
    ...
    result_datas.append(datas_frid)
    result_datas.append(datas_ir)
    result_datas_json = json.dumps(result_datas)
    return HttpResponse(result_datas_json)

def is_warning(rsg):
    room = rsg.room
    strategys = room.strategys.all()
...
def ReportRoomStatus(request):
    ...
    if is_add and room_obj:
        try:
            ...
            ret = is_warning(rsg)
            rsg.is_warning = ret[2]
            rsg.save()
            if rsg.is_warning:
                msg = do_warning(room_obj[0].roomsn)
                print msg
            else:
                msg = "ok"
        except Exception, e:
