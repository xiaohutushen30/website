...
class AddVisitorForm(forms.ModelForm):
    class Meta:
        model = Visitor
        fields = ('personsn','username','phone','sex','is_staff')
        widgets = {
            'is_staff' : forms.RadioSelect(choices=((u'True', u'内部员工'),(u'False', u'外部员工')),attrs={'class':'list-inline'}),
            'personsn': forms.Select(choices=[(x.personsn,x.personsn) for x in VisitorTemporary.objects.all()],attrs={'class':'form-control'}),
...
class EditVisitorForm(forms.ModelForm):
    class Meta:
        model = Visitor
        fields = ('personsn','username','phone','sex','is_bind','is_staff')
        widgets = {
            'is_staff' : forms.RadioSelect(choices=((u'True', u'内部员工'),(u'False', u'外部员工')),attrs={'class':'list-inline'}),
            'personsn': forms.Select(choices=[(x.personsn,x.personsn) for x in VisitorTemporary.objects.all()],attrs={'class':'form-control'}),
            'username' : forms.TextInput(attrs={'class':'form-control'}),
